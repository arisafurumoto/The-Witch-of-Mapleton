...
# Sage's static Mapleton Lane delivery interaction for the tiny 1.2 slice.
# Kept separate from his shop visitor script to avoid growing an NPC framework.

@export var quest_id: String = "sage_seedling_restock"
@export var home_facing: String = "east"
...
func interact() -> void:
	...
	if state == QuestSystem.STATE_COMPLETED:
		await _say(speaker_name, _quest_lines(quest, "post_complete_lines", [
			"The seedlings are doing nicely. Gently is usually the right speed.",
		]))
		...
		await _say(speaker_name, _quest_lines(quest, "not_started_lines", [
			"I pinned a small restock note to the board, if you are taking town errands.",
		]))
